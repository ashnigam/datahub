...
def generate_schema_file_from_avro_schema(input_file: str, platform: str, output_file: str, to_file: bool, to_server: bool):
    ...
    with open(avro_schema_file) as f:
        ...
        print(f'Schema canonical form: Length ({len(canonical_form)}); {canonical_form}')
        md5_bytes = avro_schema.fingerprint('md5')
        avro_schema_hash = md5_bytes.hex()
        assert isinstance(avro_schema, RecordSchema), 'This command only works for Avro records'
        dataset_urn = make_dataset_urn(platform=platform_urn, name=f'{avro_schema.namespace}.{avro_schema.name}' if avro_schema.namespace else avro_schema.name)
        schema_fields = [f for f in converter.to_mce_fields(avro_schema, is_key_schema=False)]
        schema_metadata = models.SchemaMetadataClass(schemaName=avro_schema.name, platform=platform_urn, version=0, hash=avro_schema_hash, platformSchema=models.OtherSchemaClass(rawSchema=raw_string), fields=schema_fields)
        assert schema_metadata.validate()
        if to_file:
            with SynchronizedFileEmitter(output_file_name) as file_emitter:
                file_emitter.emit(MetadataChangeProposalWrapper(entityUrn=dataset_urn, aspect=schema_metadata))
        if to_server:
            with get_default_graph() as graph:
                graph.emit(MetadataChangeProposalWrapper(entityUrn=dataset_urn, aspect=schema_metadata))
