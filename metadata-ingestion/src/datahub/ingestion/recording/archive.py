...
import yaml
from datahub.ingestion.recording.config import REPLAY_DUMMY_MARKER, REPLAY_DUMMY_VALUE, check_recording_dependencies
...
HTTP_DIR = 'http'
DB_DIR = 'db'
SECRET_PATTERNS = ['.*password.*', '.*secret.*', '.*token.*', '.*api[_-]?key.*', '.*private[_-]?key.*', '.*access[_-]?key.*', '.*credential.*']
NON_SECRET_FIELDS = ['authentication_type', 'authenticator', 'auth_type', 'auth_method', 'authorization_type']
...
def _replace_markers_recursive(obj: Any, parent_key: str='') -> Any:
    ...
    """
    if isinstance(obj, dict):
        return {key: _replace_markers_recursive(value, key) for key, value in obj.items()}
    if isinstance(obj, list):
...
class ArchiveManifest:
    ...
    def __init__(self, run_id: str, source_type: Optional[str]=None, sink_type: Optional[str]=None, datahub_cli_version: Optional[str]=None, python_version: Optional[str]=None, created_at: Optional[str]=None, checksums: Optional[Dict[str, str]]=None, format_version: str=ARCHIVE_FORMAT_VERSION, exception_info: Optional[Dict[str, Any]]=None, recording_start_time: Optional[str]=None) -> None:
        self.run_id = run_id
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {'format_version': self.format_version, 'run_id': self.run_id, 'source_type': self.source_type, 'sink_type': self.sink_type, 'datahub_cli_version': self.datahub_cli_version, 'python_version': self.python_version, 'created_at': self.created_at, 'checksums': self.checksums, 'recording_start_time': self.recording_start_time}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArchiveManifest':
        ...
        return cls(run_id=data['run_id'], source_type=data.get('source_type'), sink_type=data.get('sink_type'), datahub_cli_version=data.get('datahub_cli_version'), python_version=data.get('python_version'), created_at=data.get('created_at'), checksums=data.get('checksums', {}), format_version=data.get('format_version', ARCHIVE_FORMAT_VERSION), exception_info=data.get('exception_info'), recording_start_time=data.get('recording_start_time'))
...
class RecordingArchive:
    ...
    def create(self, output_path: Path, temp_dir: Path, manifest: ArchiveManifest, recipe: Dict[str, Any], redact_secrets_enabled: bool=True) -> Path:
        ...
        if redact_secrets_enabled:
            ...
            logger.warning('Secret redaction DISABLED - recipe contains actual credentials. Do NOT share this recording outside secure environments.')
...
def get_archive_info(archive_path: Path, password: str) -> Dict[str, Any]:
    ...
    info = {'format_version': manifest.format_version, 'run_id': manifest.run_id, 'source_type': manifest.source_type, 'sink_type': manifest.sink_type, 'datahub_cli_version': manifest.datahub_cli_version, 'python_version': manifest.python_version, 'created_at': manifest.created_at, 'recording_start_time': manifest.recording_start_time, 'file_count': len(manifest.checksums), 'files': files, 'has_exception': manifest.has_exception, 'has_http_cassette': has_http_cassette, 'has_db_queries': has_db_queries}
